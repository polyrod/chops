...
fetchDecodeExecuteLoop = do
  (ln,ins) <- fetchIns
  when debug $ liftIO $ displayIns (ln,ins)
  exec ins
  operateVM
...
displayIns (ni,i) = do
                          ...
                          mapM_ (putStr . formatToString string) ps
